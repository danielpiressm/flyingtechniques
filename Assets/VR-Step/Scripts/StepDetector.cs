...
namespace VRStep
{
	...
	public class StepDetector : MonoBehaviour
	{
		...
		void Update()
		{
            ...
            inVector = camera.transform.localPosition;
            Math3d.LinearAcceleration(out outVector, inVector, 10);/*GetUpwardsAcceleration(Input.gyro.userAcceleration);*/
            float upVector = Vector3.Dot(camera.transform.up, outVector);
            Debug.Log("out = " + upVector + " : " + outVector);
